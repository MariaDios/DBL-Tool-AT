...
DIM_DESCRIPTIONS = {
    "Data Categories & Content Scope":  "Assesses what types of building data the system records — energy, technical, and renovation information.",
    "Circularity-related Information":  "Measures how well the system supports circular economy goals through material tracking and waste reduction.",
    "Governance Model":                 "Evaluates the legal foundation, institutional responsibility, and data rights framework of the system.",
    "Interoperability":                 "Examines how well the system connects with other digital tools, open standards, and national registries.",
    "Usability":                        "Rates accessibility for building owners, professionals, and non-expert users.",
    "Lifecycle Coverage":               "Assesses whether the system covers the full building lifecycle from design through to end-of-life.",
}
...
DEFAULT_SCORES = {
    ...
    "France — CIL": {
        ...
        "sources":{"Legal Mandate":"Loi Climat et Resilience (2021), Art. 167","Material Passport":"No material passport documented","Open API / Data Exchange":"No open API in French Government sources"},
    },
    "Germany — iSFP": {
...
<div style="background:linear-gradient(135deg,#1a3a2a 0%,#2d5a40 100%);border-radius:14px;padding:2.5rem 3rem;margin-bottom:1.5rem">
  <div style="color:#FFFFFF;font-size:2.2rem;font-weight:800;margin-bottom:0.6rem;line-height:1.2;font-family:Segoe UI,Arial,sans-serif">DBL Tool AT — Digital Building Logbook Evaluation Tool for Austria</div>
  <div style="color:#FFFFFF;font-size:1.1rem;margin:0;font-weight:400;font-family:Segoe UI,Arial,sans-serif">Comparative analysis of 6 European DBL systems &nbsp;&middot;&nbsp; 6 dimensions &nbsp;&middot;&nbsp; 18 criteria &nbsp;&middot;&nbsp; Master's Thesis &nbsp;&middot;&nbsp; Maria Dioszegi &nbsp;&middot;&nbsp; UAS Technikum Wien 2026</div>
</div>
""", unsafe_allow_html=True)
...
# Tab order: Profiles, Edit Scores, Comparison Table, Recommendations
...
with tabs[0]:
    ...
    st.markdown('<p style="color:#444444;font-size:13px">Click a system to expand its profile. Scores are pre-filled from document analysis.</p>', unsafe_allow_html=True)
    ...
    st.markdown('<p class="dim-desc">Average score per comparison dimension (1 = highest fulfilment, 5 = absent). Each dimension groups three criteria of equal weight. Lower scores indicate stronger implementation.</p>', unsafe_allow_html=True)
...
with tabs[1]:
    ...
    st.markdown('<p style="color:#444444;font-size:13px">Adjust scores using the sliders below. When finished, click <b>Save all scores</b> at the bottom. The page will not update until you save.</p>', unsafe_allow_html=True)
...
with tabs[3]:
    ...
    st.markdown('<p style="color:#111111;font-size:13px">Each requirement is derived from the highest-scoring approach identified across the six systems. Click a criterion to expand. To save as PDF: Ctrl+P in browser → Save as PDF.</p>', unsafe_allow_html=True)
    ...
    for dim, crits in DIMENSIONS.items():
        ...
        padding:0.5rem 1rem;margin:1rem 0 0.3rem;font-weight:700;color:#1a3a2a;font-size:1rem">
        {dim}
        </div>
        """, unsafe_allow_html=True)
        for c in crits:
